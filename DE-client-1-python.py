...
def client(router_ip, router_port, server_ip, server_port):
    client_ip = server_ip
    client_port = 1220 ## dunno what the client port should be but we have to include in message sent...
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        with open(0, "rb") as fd:
            while True:
                ...
                if not data: break
                sys.stdout.buffer.raw.write(data) ##may only write partial messages to output
